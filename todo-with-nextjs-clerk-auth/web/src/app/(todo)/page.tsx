...
import TodoList, { TodoDetails, TodoDetailsWithId } from "@/components/todo/TodoList";
...
export default function Page() {
  ...
  const refetchQuery = {
    refetchQueries: [{ query: todoQuery.query, variables: todoQuery.variables }],
  };

  const { data, refetch } = useQuery(todoQuery.query, {
  ...
  const commonClasses =
    "mr-3 ml-3 lg:mr-6 lg:ml-6 flex h-10 w-24 items-center justify-center rounded-md text-xl dark:text-white";
  ...
  return (
    <div className="flex flex-col h-screen items-center justify-between p-2 pt-24 pb-6">
      ...
      <TodoList key={status} status={status} todos={todos} refetchQuery={refetchQuery} />
    </div>
  );
}
